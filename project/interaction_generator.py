...
基于用户兴趣生成用户-商品交互数据
Generate user-item interaction data based on user interests
"""
...
class InteractionGenerator:
    """交互生成器 / Interaction Generator"""
    
    def __init__(self):
        """初始化交互生成器 / Initialize the interaction generator"""
        ...
        # 存储生成的交互数据 / Store generated interaction data
        self.interactions_df = None
        self.user_item_interactions = {}  # 用户-商品交互记录 / User-item interaction records
    
    #region 生成交互类型 / Generate interaction type
    def generate_interaction_type(self, user_id: str, item_category: str, 
                                user_generator: UserGenerator) -> str:
        """
        基于用户兴趣生成交互类型
        Generate interaction type based on user interest
        Args:
            user_id: 用户ID / User ID
            item_category: 商品分类 / Item category
            user_generator: 用户生成器实例 / User generator instance
            
        Returns:
            交互类型 ('click', 'cart', 'favorite') / Interaction type ('click', 'cart', 'favorite')
        """
        # 获取用户对该分类的交互概率 / Get user interaction probability for this category
        base_probability = user_generator.get_user_interaction_probability(user_id, item_category)
        
        # 基于基础概率决定是否生成交互 / Decide whether to generate interaction based on base probability
        if random.random() > base_probability:
            return None  # 不生成交互 / Do not generate interaction
        
        # 基于交互类型分布生成交互类型（所有用户都有机会生成不同类型） / Generate interaction type based on interaction type distribution (all users have a chance to generate different types)
        ...
        if random_value < self.interaction_types['click']:
            ...
            return 'favorite'
    #endregion
    #region 为指定用户生成交互数据 / Generate user interactions
    def generate_user_interactions(self, user_id: str, user_generator: UserGenerator, 
                                 item_generator: ItemGenerator) -> List[Dict]:
        """
        为指定用户生成交互数据
        Generate interaction data for a specified user
        Args:
            user_id: 用户ID / User ID
            user_generator: 用户生成器实例 / User generator instance
            item_generator: 商品生成器实例 / Item generator instance
            
        Returns:
            用户交互数据列表 / List of user interaction data
        """
        user_interactions = []
        
        # 获取用户偏好的分类 / Get user's preferred categories
        ...
        # 计算用户交互数量范围 / Calculate user interaction quantity range
        ...
        # 为每个交互选择商品 / Select item for each interaction
        for _ in range(num_interactions):
            # 选择分类（偏好分类有更高概率被选择） / Select category (preferred categories have higher probability of being selected)
            if preferred_categories and random.random() < 0.8:
                # 80%概率选择偏好分类 / 80% probability of selecting a preferred category
                if high_interest_categories and random.random() < 0.6:
                    # 60%概率选择高兴趣分类 / 60% probability of selecting a high-interest category
                    category = random.choice(high_interest_categories)
                else:
                    # 40%概率选择其他偏好分类 / 40% probability of selecting other preferred categories
                    category = random.choice(preferred_categories)
            else:
                # 20%概率随机选择分类（探索行为） / 20% probability of randomly selecting a category (exploratory behavior)
                all_categories = list(item_generator.category_items.keys())
                category = random.choice(all_categories)
            
            # 从分类中选择商品 / Select item from category
    ...
    #endregion
    #region 生成所有用户的交互数据 / Generate all interactions
    def generate_all_interactions(self, user_generator: UserGenerator, 
                                item_generator: ItemGenerator) -> pd.DataFrame:
        """
        生成所有用户的交互数据
        Generate interaction data for all users
        Args:
            user_generator: 用户生成器实例 / User generator instance
            item_generator: 商品生成器实例 / Item generator instance
            
        Returns:
            交互数据DataFrame / Interaction data DataFrame
        """
        print(f"正在生成交互数据... / Generating interaction data...")
        
        all_interactions = []
        num_users = self.config['num_users']
        
        # 为每个用户生成交互 / Generate interactions for each user
        for i in range(num_users):
            ...
            if (i + 1) % 100 == 0:
                print(f"  已处理 {i+1}/{num_users} 个用户 /   Processed {i+1}/{num_users} users")
        
        # 创建DataFrame / Create DataFrame
        self.interactions_df = pd.DataFrame(all_interactions)
        
        # 添加时间戳（简化处理，不模拟真实时间序列） / Add timestamp (simplified processing, not simulating real time series)
        ...
        print(f"交互数据生成完成: {len(self.interactions_df)} 个交互 / Interaction data generation complete: {len(self.interactions_df)} interactions")
        return self.interactions_df
    #endregion
    #region 获取交互统计信息 / Get interaction statistics
    def get_interaction_statistics(self) -> Dict:
        """
        获取交互统计信息
        Get interaction statistics
        Returns:
            交互统计字典 / Interaction statistics dictionary
        """
        ...
        # 用户交互数量分布 / User interaction count distribution
        ...
        # 商品交互数量分布 / Item interaction count distribution
        ...
        # 分类交互分布 / Category interaction distribution
        ...
        return stats
    #endregion
    #region 打印交互统计信息 / Print interaction statistics 
    def print_interaction_statistics(self):
        """打印交互统计信息 / Print interaction statistics"""
        stats = self.get_interaction_statistics()
        
        print(f"\n=== 交互数据统计 / Interaction Data Statistics ===")
        ...
        # 用户交互统计 / User interaction statistics
        user_stats = stats['user_interaction_stats']
        print(f"\n用户交互统计: / User Interaction Statistics:")
        print(f"  最少交互数: {user_stats['min']} /   Minimum interactions: {user_stats['min']}")
        print(f"  最多交互数: {user_stats['max']} /   Maximum interactions: {user_stats['max']}")
        print(f"  平均交互数: {user_stats['mean']:.1f} /   Average interactions: {user_stats['mean']:.1f}")
        print(f"  标准差: {user_stats['std']:.1f} /   Standard deviation: {user_stats['std']:.1f}")
        
        # 商品交互统计 / Item interaction statistics
        item_stats = stats['item_interaction_stats']
        print(f"\n商品交互统计: / Item Interaction Statistics:")
        print(f"  最少交互数: {item_stats['min']} /   Minimum interactions: {item_stats['min']}")
        print(f"  最多交互数: {item_stats['max']} /   Maximum interactions: {item_stats['max']}")
        print(f"  平均交互数: {item_stats['mean']:.1f} /   Average interactions: {item_stats['mean']:.1f}")
        print(f"  标准差: {item_stats['std']:.1f} /   Standard deviation: {item_stats['std']:.1f}")
        
        # 分类交互分布（前10个） / Category interaction distribution (Top 10)
        print(f"\n分类交互分布 (前10个): / Category Interaction Distribution (Top 10):")
        ...
        for category, count in sorted_categories[:10]:
            percentage = count / len(self.interactions_df) * 100
            print(f"  {category}: {count} ({percentage:.1f}%) /   {category}: {count} ({percentage:.1f}%)")
    #endregion
    #region 获取指定用户的交互历史 / Get user interaction history
    def get_user_interaction_history(self, user_id: str) -> pd.DataFrame:
        """
        获取指定用户的交互历史
        Get interaction history for a specified user
        Args:
            user_id: 用户ID / User ID
            
        Returns:
            用户交互历史DataFrame / User interaction history DataFrame
        """
        if self.interactions_df is None:
            return pd.DataFrame()
    ...
    #endregion
    #region 获取指定商品的交互历史 / Get item interaction history
    def get_item_interaction_history(self, item_id: str) -> pd.DataFrame:
        """
        获取指定商品的交互历史
        Get interaction history for a specified item
        Args:
            item_id: 商品ID / Item ID
            
        Returns:
            商品交互历史DataFrame / Item interaction history DataFrame
        """
        if self.interactions_df is None:
            return pd.DataFrame()
